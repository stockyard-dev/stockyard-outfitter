...
async function load(){var r=await fetch(A+'/new_hires').then(function(r){return r.json()});items=r.new_hires||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.email||'').toLowerCase().includes(q)||(i.department||'').toLowerCase().includes(q)||(i.start_date||'').toLowerCase().includes(q)});
...
if(i.email)h+='<div class="item-sub">'+esc(i.email)+'</div>';
...
if(i.department)h+='<span>'+esc(i.department)+'</span>';
if(i.start_date)h+='<span>'+esc(i.start_date)+'</span>';
if(i.manager)h+='<span>'+esc(i.manager)+'</span>';
if(i.progress)h+='<span>'+i.progress+'</span>';
...
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/new_hires/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{name:"",email:"",department:"",start_date:"",manager:"",progress:0,status:"",notes:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' OUTFITTER</h2>';
...
h+='<div class="fr"><label>Department</label><input id="f-department" value="'+esc(i.department)+'"></div>';
h+='<div class="fr"><label>Start Date</label><input id="f-start-date" type="date" value="'+esc(i.start_date)+'"></div>';
h+='<div class="fr"><label>Manager</label><input id="f-manager" value="'+esc(i.manager)+'"></div>';
h+='<div class="fr"><label>Progress</label><input id="f-progress" type="number" value="'+(i.progress||0)+'"></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),email:document.getElementById('f-email').value.trim(),department:document.getElementById('f-department').value.trim(),start_date:document.getElementById('f-start-date').value.trim(),manager:document.getElementById('f-manager').value.trim(),progress:parseInt(document.getElementById('f-progress').value)||0,status:document.getElementById('f-status').value.trim(),notes:document.getElementById('f-notes').value.trim()};
if(editId){await fetch(A+'/new_hires/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/new_hires',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
